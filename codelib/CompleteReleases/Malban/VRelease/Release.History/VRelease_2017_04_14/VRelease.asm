; shield and full squadron hits = pacman?
; this file is part of Release, written by Malban in 2017
;
;                    bank     1 
INVINCIBLE          =        0 
; glitch while explosion is removed - SOMETIMES?
; 
; bomber nicht alleine! Endless possible
; Pacman
; scroll text does not start corretly when right is changed from 127
; bonus stars may have different dirtections
; possible bonus ALL STAR if no star athere?
; Sound missing:
; spawn hiden X/star
...
release_string 
                    DB       "RELEASE B1", $80            ; some game information, ending with $80
...
backFromDemo 
                    ...
                    LDA      #-$80                        ; y position of scroller 
                    STA      scroll_y                     ; store it 
                    LDA      #-128                        ; left boundary 
                    STA      scroll_x_left                ; store it 
                    LDA      #127                         ; right boundary 
...
title_main1_hs_ret 
                    jsr      initTitle2 
title_main1 
                    jsr      decode_ym_1_channel          ; the ym song only using channel 1 
                    jsr      Wait_Recal 
...
do_normal_after_all 
                    ...
                    pulu     d,x,y,pc                     ; (D = y,x, X = vectorlist, Y = DDRA+Scale) ; do all objects (the circling letters and the starfield) 
check_buttons 
...
; reset music to 0
                    ...
                    INIT_MUSICl  inGameMusic1 
main:                                                     ;#isfunction  
                    jsr      decode_on                    ; decode the possible rest of un decoded YM stuff (if there are not enought objetcs with move vectors) 
...
; spawn 6
                    ...
                    pshs     x 
                    jsr      spb_allowed 
                    bsr      correctSquad                 ;CORRECT_SQUAD 
                    jsr      spb_allowed 
...
exchangeToExplosion:                                      ;#isfunction  
                    pshs     u,x 
                    lda      TYPE, u 
                    cmpa     #TYPE_DRAGON+$40             ; check if we hit a "full life" dragon 
                    bne      nofullDragon_ete 
                    anda     #%00111111                   ; if yes, delete the full life flag of the dragon 
                    sta      TYPE, u 
...
; copy interesting things from y to u
                    lda      #TYPE_SHOT 
                    sta      TYPE, u 
                    ldd      Y_POS+u_offset1,y            ; get position of parent (bomber) 
                    std      Y_POS,u                      ; and remember it in the shot object 
...
;***************************************************************************
                    ...
                    INCLUDE  "title.asm"                  ; 
                    INCLUDE  "options.asm"                ; 
                    INCLUDE  "gameover.asm"               ; s
scroller1: 
                    DB       "                ORIGINAL BY GIMO GAMES.                   ",$80
scroller2: 
                    db       "                VECTREX VERSION PROGRAMMED BY MALBAN. MUSIC COMPOSED BY VTK.                   ",$80
scroller3: 
                    db       "                ORIGINAL SERIAL ROUTINES BY ALEX HERBERT. ADAPTED SERIAL ROUTINES BY THOMAS SONTOWSKI. RELEASE FITTED SERIAL ROUTINES BY MALBAN. HAVE A NICE GAME.                   ", $80
;***************************************************************************
;***************************************************************************
;***************************************************************************
CURRENT_BANK        =        2 
