...
Dragon_add_delay         ds       1                            ; change pos each this "ticks" 
...
Hunter_add_delay    ds       1                            ; change pos each this "ticks" 
Bomber_addi         ds       1                            ; change pos by this each "change" 
Bomber_add_delay    ds       1                            ; change pos each this "ticks" 
base_angle          ds       2                            ; current start angle of base (*2) 
shot_addi           ds       1                            ; change pos by this each "change" 
shot_add_delay      ds       1                            ; change pos each this "ticks" 
...
;
                    ...
                    ds       PREVIOUS_OBJECT,2            ; positive = start of list 
                    ds       NEXT_OBJECT,2                ; positive = end of list 
                    ds       ANIM_COUNTER,1               ; with what value does the movement get updated (1-4)? 
...
;
                    ...
                    ds       SHOT_COUNTER_RESET,1         ; with what value does the movement get updated (1-4)? 
                    ds       SHOT_COUNTER,1               ; with what value does the movement get updated (1-4)? 
...
INITIAL_SHOT_MOVE_STRENGTH  =  1                          ; move three scale pos 
...
INITIAL_DRAGON_MOVE_DELAY  =      5                            ; every second tick 
DRAGON_CHILD_FREE_SPEED = 2

SHIELD_DELAY_TICKS = 7
SPAWN_INCREASE_DELAY = 5
...
no_underflow_os: 
                    jsr      getDivi_b 
                    cmpb     SCALE,u                      ; compare outer border with object position 
                    lbhs     out_notTouched               ; branch if a (shield inner wall) is higher or same than pos (scale) of object 
                    lda      #1 
                    rts      
...
initGame                                                  ;#isfunction  
 ...
 sta shield_delay
                    ...
                    lda      #INITIAL_DRAGON_MOVE_DELAY 
                    sta      Dragon_add_delay                  ; update x every cycle 

                    lda      #INITIAL_X_MOVE_DELAY 
                    sta      X_add_delay                  ; update x every cycle 
                    lda      #INITIAL_X_MOVE_STRENGTH 
                    sta      X_addi                       ; with one scale dec 
                    lda      #INITIAL_SHOT_MOVE_DELAY 
                    sta      shot_add_delay               ; update x every cycle 
                    lda      #INITIAL_SHOT_MOVE_STRENGTH 
                    sta      shot_addi                    ; with one scale dec 
                    lda      #INITIAL_HUNTER_MOVE_DELAY 
                    sta      Hunter_add_delay             ; update x every cycle 
                    lda      #INITIAL_HUNTER_MOVE_STRENGTH 
                    sta      Hunter_addi                  ; with one scale dec 
                    lda      #INITIAL_BOMBER_MOVE_DELAY 
                    sta      Bomber_add_delay             ; update x every cycle 
                    lda      #INITIAL_BOMBER_MOVE_STRENGTH 
                    sta      Bomber_addi                  ; with one scale dec 
