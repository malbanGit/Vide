...
list_empty_head     ds       2 
list_objects_head   ds       2 
list_objects_tail   ds       2 

MAX_OBJECTS = 30 ; todo determine max possible spawns!
...
; if NEXT_OBJECT is null, than the list is finished
                    org      $c900 
; 21 bytes per object
                    struct   ObjectStruct 
                    ds       BEHAVIOUR,2 
                    ...
                    ds       PREVIOUS_OBJECT,2            ; null = start of list 
                    ds       NEXT_OBJECT,2                ; null = end of list 
                    ds       filler, 5 
                    end struct 
; all following object "inherit" from above defined Object struct
; all vars after "DRAW_ROUTINE" can be different for each of the objects
; (up to PREVIOUS_OBJECT)
;
...
o_draw              =        object_list+DRAW_ROUTINE 
...
; calculate object position of next spawned object
 ldu list_empty_head
 beq cs_done ; we don't have any spare objects -> go out
...
; of our new object, that last object is the previous
 ...
 beq no_next_cs; the last object was 0, so we do net set a next there
...
; random coordinates
; scale
; ... somehow determine random type of object
