...
I have not tested any other assembler - but the project might also be assembled using the
...
Configuration
-------------
At the start of VRelease.asm to configuration options can be set.
a) INVINCIBLE [default is 0]
- if defined = 1, than the player can not die (- but you actually can not quite!)
...
- in general means, that if not defined as 1 
  ...
  This also means the resulting "bin" file is 2 bytes shorter ( :-) ) - and it is 
...
- ds2431XXX and eprom.i
  Contain routines to access the 2431 one wire eprom chip.
...
- inBothBanksXXX
  ...
  * in two used to be bankswitching routines, but as of now only the VecFever routines reside there
...
- objectsXXX
  ...
  * 2 contains all "friendly objects
...
- scroller.asm
  ...
  and hardcoded the borders (although the variable are still in the code and still used - but they
...
..........
- abc.i
  Contains the font definition that is used throught the game to show text. The Font is "special" in the
  way, that each "length" is dividable by 8, this makes clipping possible - using the above mentioned package.
