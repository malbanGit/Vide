...
#define jsrdu(d, pointer, func, comment)   asm("ldu %01\n\t" \
                                "ldd %00\t;  "#comment" \n\t" \
                                "jsr " #func "\n\t" : : "g" (d), "g" (pointer) : "d", "u", "x")
#define jsrabu(a, b, pointer, func)  jsrdu( (long) ( (((long)a)*((long)256))+((long)b)) , pointer, func, (y=#a, x=#b))

#define jsr_reta(func)           asm("jsr " #func "\n\t" : : : "d", "x")

static volatile unsigned char __A;
#define _random()             asm volatile ("jsr 0xF517\n\tsta ___A" : : :"d", "x")   // 
...
#define read_buttons()    jsra(0, 0xF1B4) 
...
#define draw_dot(x, y)              jsrba(x, y, 0xF2C3) //jsr(0xF2C3)   // 
#define Dot_d()              jsrba(x, y, 0xF2C3) //jsr(0xF2C3)   // 
...
#define move_to(x, y)       jsrba(x, y, 0xF312)   // 
...
#define print_str(y,x,p)    jsrabu(y,x,p, 0xF37a) 
...
*/
#define draw_vector_list(c)          jsrx(c, 0xF3CE)   // count y x y x ...
...
#define recalculate_music(m)    jsru(m, 0xF687)   // 
