...
//			System.out.println("New overlapLength: " + overlapLength+"(old: "+oldOverlapLength+")");
		}
	...
	*/
	...
	public boolean process(AudioEvent audioEvent) {
		...
		int sequenceLength = seekWindowLength - 2 * overlapLength;
                ...
                // code by malban
                // using "small"  buffers
                // the below code was neccessary to 
                // prevent exceptions
                // 
                int copyMax1=sequenceLength;
                if (audioFloatBuffer.length <offset + overlapLength+sequenceLength)
                {
                    copyMax1 = sequenceLength-( (offset + overlapLength+sequenceLength)-audioFloatBuffer.length);
                    sequenceLength = copyMax1;
                }
        ...
        // processing sequence and so on
                ...
                // prevent exceptions
                // 
                int copyMax = overlapLength;
                if (audioFloatBuffer.length < offset + sequenceLength + overlapLength*2)
                {
                    copyMax = overlapLength -((offset + sequenceLength + overlapLength*2) -audioFloatBuffer.length);
                    for (int i=copyMax; i<overlapLength; i++) pMidBuffer[i]=0;
                }
                
		System.arraycopy(audioFloatBuffer, offset + sequenceLength + overlapLength, pMidBuffer, 0, copyMax);
