...
public class mod2VectrexOrg 
{
    ...
    void show_message()
    {
        System.out.println(PROGNAME+" v"+VERSION+" - "+_DATE+" by "+AUTHOR);
        System.out.println();
    }
    ...
    void readMod(String fileName)
    {
        ...
        int pos = 0;
        if (VERBOSITY>0) System.out.println("Reading songName...");
        try 
        {
        ...
        if (VERBOSITY>0) System.out.println("Reading Samples...");
        ...
        //songlength
        if (VERBOSITY>0) System.out.println("Reading songLength...");
        if (pos<a.length) songlength = a[(pos++)];
        if (pos<a.length) data[0] = a[(pos++)];

        // pattern position table
        if (VERBOSITY>0) System.out.println("Reading Pattern Position Table...");
        for (c=0; c<=127;c++)
            if (pos<a.length) pattern_position[c]= a[(pos++)];
        findPatternNumber();

        // "4chn"
        if (VERBOSITY>0) System.out.println("Reading Tracker-ID string...");
        trackerID = "";
        for (d = 1;d<=4; d++)
        {
            swapChar = 0;
        ...
        loop_position = 0;
        if (VERBOSITY>0) System.out.println("Reading Patterns...");
        for (f= 0; f<=pattern_number;f++)
        {
            if (VERBOSITY>1) System.out.print(" Pattern "+String.format("%02d", f)+": ");
            for (c = 0;c<=63;c++)
            {
                ...
                for (d= 0;d<=3;d++) 
                {
                    ...
                    {
                        if (loop_position != 0) System.out.print("! >> Caution: More than one loop found in modfile, so I am taking the last one.");
        ...
        if (VERBOSITY>1) System.out.println("done.");
        if (VERBOSITY>1) System.out.println("");
    }
    ...
    void writeModInfo(String fileName)
    {
        ...
        try
        {
            ...
            if (VERBOSITY>0) System.out.println(" Writing header...");
            writer.println(PROGNAME+" v"+VERSION+" - "+_DATE+" by "+AUTHOR);
            writer.println("");
            if (VERBOSITY>0) System.out.println(" Writing SongName...");
            writer.println("SongName: \""+songName+"\"");
            writer.println("");
            if (VERBOSITY>0) System.out.println(" Writing TrackerID...");
            writer.println("TrackerID: \""+trackerID+"\"");
            writer.println("");
            writer.println("");

            if (VERBOSITY>0) System.out.println(" Writing Sample Information...");
            ...
            if (VERBOSITY>0) System.out.println(" Writing SongLength...");
            ...
            if (VERBOSITY>0) System.out.println(" Writing Pattern Position Table...");
            ...
            if (VERBOSITY>0) System.out.println(" Writing Patterns...");
            for (e=0; e<=pattern_number; e++)
            {
                if (pattern[e].used) 
                {
                    ...
                    writer.println("");
                    if (VERBOSITY>1) System.out.print("  Pattern "+hex((byte)e)+": ");
                    for (c=0;c<=63; c++)
                    {
            ...
            if (VERBOSITY>0) System.out.println(" Writing Protracker Effect Information...");
            for (e=0; e<=35;e++)
    ...
    void writePattern(PrintWriter writer)
    {
        ...
        System.out.println("Writing section \"Patterns\"");
            ...
            if (pattern[e].used)
            {
               ...
               for (c = 0; c<=63;c++)
               {
                   ...
                   for (d = 0;d<=2;d++) 	// first three voices 0-2
                   {
                       ...
                       {
                           if (VERBOSITY>0) System.out.println("Setting speed to "+hex((byte)(pattern[e].voice[d].tone[c].effect2*16+pattern[e].voice[d].tone[c].effect3)));
                       ...
                       if (data<(2*12)) 
                           System.out.println("*** Attention: Pattern $"+hex((byte)e)+" Voice $"+hex((byte)d)+" Position $"+hex((byte)c)+" is too low! ***");
                       else 
    ...
    void write (String fileName)
    {
        ...
        try
        {
            ...
            System.out.println("Writing file \""+fileName+"\" beginning from "+header_string);
            System.out.println();
            writePattern(writer);
        ...
        System.out.println("The first three channels of the mod file have been converted (0-2).");
        System.out.println("Every voice in a pattern has the length of $40 bytes.");
        System.out.println("Every pattern is 3x$40 bytes = $c0 bytes long.");
       // System.out.println("Offnotes have the value of ",OFFNOTE_VALUE,".");
        System.out.println();
        System.out.println("Speed is set to $"+hex(SPEED)+".");
        System.out.println();
        System.out.println("Instrument 1 is a bass drum and gets value $"+hex(BASS_DRUM_VALUE)+".");
        System.out.println("Instrument 2 is a hihat drum and gets value $"+hex(HIHAT_DRUM_VALUE)+".");
        System.out.println("Instrument 3 is a snare drum and gets value $"+hex(SNARE_DRUM_VALUE)+".");
        System.out.println();
       // System.out.println("Instrument 4 makes Vectrex Soundchip silent (value $"+hex(SILENCE_VALUE)+").");
        System.out.println();
        System.out.println("Lowest note should be G-2.");
        System.out.println();    
    }
    
    public boolean doIt(String filename)
    {
        for (int i=0; i< 128; i++)
        {
