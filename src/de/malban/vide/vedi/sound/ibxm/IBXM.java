...
public class IBXM {
	...
	private static final int OVERSAMPLE = 2;
        ...
        public static int IBXM_MAXBUFFER = ( 44100 * OVERSAMPLE * 5 / 32 )+ ( 256 * 2 );
	...
	*/
	public int getAudio( int[] outputBuffer ) {
		...
		for( int chanIdx = 0; chanIdx < module.numChannels; chanIdx++ ) {
                    if (playVoices.length>chanIdx)
                    {
                        if (!playVoices[chanIdx]) continue;
                    }
        ...
        {
            playVoices = new boolean[voicePlay.length];
            int c = 0;
