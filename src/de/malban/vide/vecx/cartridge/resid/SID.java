...
import de.malban.vide.vecx.cartridge.resid.ISIDDefs.chip_model;
...
public class SID implements Serializable{
	...
	public class State implements Serializable{
            ...
            {
                State s= new State();
                for (int i=0;i<0x20;i++) s.sid_register[i] = sid_register[i];
                s.bus_value = bus_value;
                s.bus_value_ttl = bus_value_ttl;
                for (int i=0;i<3;i++) s.accumulator[i] = accumulator[i];
                for (int i=0;i<3;i++) s.shift_register[i] = shift_register[i];
                for (int i=0;i<3;i++) s.rate_counter[i] = rate_counter[i];
                for (int i=0;i<3;i++) s.rate_counter_period[i] = rate_counter_period[i];
                for (int i=0;i<3;i++) s.exponential_counter[i] = exponential_counter[i];
                for (int i=0;i<3;i++) s.exponential_counter_period[i] = exponential_counter_period[i];
                for (int i=0;i<3;i++) s.envelope_counter[i] = envelope_counter[i];
                for (int i=0;i<3;i++) s.envelope_state[i] = envelope_state[i];
                for (int i=0;i<3;i++) s.hold_zero[i] = hold_zero[i];
                return s;
            }
	...
	transient protected Voice voice[] = new Voice[] { new Voice(), new Voice(),
			new Voice() };

	transient public Filter filter = new Filter();

	transient protected ExternalFilter extfilt = new ExternalFilter();

	transient protected Potentiometer potx = new Potentiometer();

	transient protected Potentiometer poty = new Potentiometer();

	transient protected int /* reg8 */bus_value;

	transient protected int /* cycle_count */bus_value_ttl;

	transient protected double clock_frequency;

	/**
	 * External audio input.
	 */
	transient protected int ext_in;
	 ...
	 */
	transient protected static final int FIR_N = 125;
	 ...
	 * resolution of 16 bits this yields L >= 285 and L >= 51473, respectively.
	 */
	transient protected static final int FIR_RES_INTERPOLATE = 285;
	 ...
	 * resolution of 16 bits this yields L >= 285 and L >= 51473, respectively.
	 */
	transient protected static final int FIR_RES_FAST = 51473;
	 ...
	 * resolution of 16 bits this yields L >= 285 and L >= 51473, respectively.
	 */
	transient protected static final int FIR_SHIFT = 15;
	 ...
	 * resolution of 16 bits this yields L >= 285 and L >= 51473, respectively.
	 */
	transient protected static final int RINGSIZE = 16384;

	/**
	 * Fixpoint constants (16.16 bits).
	 */
	transient protected static final int FIXP_SHIFT = 16;

	/**
	 * Fixpoint constants (16.16 bits).
	 */
	transient protected static final int FIXP_MASK = 0xffff;

	/**
	 * Sampling variables.
	 */
	transient protected sampling_method sampling;

	/**
	 * Sampling variables.
	 */
	transient protected int /* cycle_count */cycles_per_sample;

	/**
	 * Sampling variables.
	 */
	transient protected int /* cycle_count */sample_offset;

	/**
	 * Sampling variables.
	 */
	transient protected int sample_index;

	/**
	 * Sampling variables.
	 */
	transient protected short sample_prev;

	/**
	 * Sampling variables.
	 */
	transient protected int fir_N;

	/**
	 * Sampling variables.
	 */
	transient protected int fir_RES;

	/**
	 * Ring buffer with overflow for contiguous storage of RINGSIZE samples.
	 */
	transient protected short sample[];

	/**
	 * FIR_RES filter tables (FIR_N*FIR_RES).
	 */
	transient protected short fir[];
	...
	public State read_state() {
		...
		for (i = 0, j = 0; i < 3; i++, j += 7) {
			...
			state.sid_register[j + 4] = (byte) ((wave.waveform << 4)
					| ((wave.test != 0) ? 0x08 : 0)
