...
import static de.malban.vide.vecx.VecXPanel.DEVICE_VECVOX;
...
public class VecSpeechDevice extends AbstractDevice implements Serializable
{
    ...
    {
        return stateNames[lowLevelState];
    }
    public String getCommand()
    {
        if (SPO256AL2)
        {
            return VecVoiceSamples.getMnemonic(lastCommand);
        }
        return VecVoxSamples.getMnemonic(lastCommand);
    }
    
    public int getQueueLength()
    {
        int tmp_nextComandPoint = nextComandPoint;
        int tmp_currentComandPoint = currentComandPoint;
        int counter = 0;
        
        while (true)
        {
            if (tmp_nextComandPoint == tmp_currentComandPoint) return counter; // no command

            int command = commands[tmp_currentComandPoint];
            tmp_currentComandPoint = (tmp_currentComandPoint + 1) % C_LEN;

            // if no command, than this is an error, nonetheless - do nothing
            if (tmp_currentComandPoint == currentComandPoint) return counter;
            if (command == -1) return counter;
            counter++;
        }
    }
    ...
    {
        if ((currentByteRead & 0x80) == 0x80) return "1";
        return "0";
    }
    
    public String getInputLine()
    {
        if (dataLineInput) return "1";
        return "0";
    }
    ...
    private int C_LEN = BUFFER_LEN_VECVOX; // Speakjet command buffer
    transient private static final int BUFFER_LEN_VECVOX = 64; // Speakjet command buffer
    transient private static final int BUFFER_LEN_VECVOICE = 32; // SP256AL command buffer
    ...
    private boolean spNextStress = false; // not implemented
    private boolean spNextRelax = false; // not implemented
    private boolean spCommandMode = false;
    private double spVolume = ((double)(96)) / 127.0;
    ...
    void reset()
    {
        doSampleBlending = false;           // per sample "decision" to do blending
        removeSilenceFromSample = true;     // per sample "decicion" to remove silence is set by emulation
        afterPause = true;                  // if directly after a pause, we do not remove silience!
        lastCommand = -1;                       // last command that was "executed"
        isVoiced = false;                   // VecVox, was the last sample "voiced", if not pitch change should not be done


        lowLevelState = LINE_OFFLINE;
        cycles = 0;
        receiveCycleStart = 0;
        dataLineInput = true; // false is 0, true is 1; bit 4 of snd_reg[14]
        old_dataLineInput = true;
        ...
        nextComandPoint = 0;
        currentComandPoint = 0;
        soundCycles = 0;
        oldCycles =0;
        currentSamplePos = -1;  // current position in in between buffer (sample -> wavData -> (wavDataLastEnd+) wavDataUsage -> soundbuffer -> line)
        wavData = null;      // buffer where samples go
        wavDataUsage = null; // buffer where sample, + lastEnd (blended or not) is combined, th 
        wavDataLastEnd = null; // buffer where the "end" of the last sample goes, needed for blending
    

        soundBytes = null; // buffer for input to TinySound line

        spNextFast = false;
        spNextSlow = false;
        spNextStress = false; // not implemented
        spNextRelax = false; // not implemented
        spCommandMode = false;
    ...
    public VecSpeechDevice clone()
    {
        ...
        vv.C_LEN = C_LEN;
        vv.spDelay = spDelay;
        vv.spNextRelax = (isSpNextRelax());
        vv.spNextStress = (isSpNextStress());
        vv.spNextFast = (isSpNextFast());
        vv.spNextSlow = (isSpNextSlow());
        vv.spCommandMode = (isSpCommandMode());
        vv.spVolume = (getSpVolume());
        vv.spCommand = (getSpCommand());
        vv.spPitch = (getSpPitch());
        vv.spTempo = (getSpTempo());
        vv.spBend = (getSpBend());
        vv.spRepeat = (getSpRepeat());
        return vv;
    }
    ...
    public void setVecVoice(boolean b)
    {
        ...
        {
            deviceName = "VecVoice";
            C_LEN = BUFFER_LEN_VECVOICE; // Speakjet command buffer
        }
        else
        {
            C_LEN = BUFFER_LEN_VECVOX; // Speakjet command buffer
            deviceName = "VecVox";
        }
        commands = new int[C_LEN];
        for (int i=0; i<C_LEN; i++) commands[i] = -1;
...
*/
    ...
    void speakjetPreprocessWavWithCommand()
    {
        ...
        spDelay = 0; // reset delay, is only initiated for debug out, delay is calculated directly as wav
        boolean enabled = false;
        
//if (enabled)
        if (getSpBend() > 5) // default 5
        {
...
// many cracks        
//if (enabled)
        if ((getSpPitch() != 88) && (isVoiced))// default 88
        {
            doTarsos = true;
            
            double tmp = 1.0/(((double)getSpPitch())/88.0);
...
// some cracks
//if (enabled)
        if (getSpTempo() != 114) // default 114
        {
            double tmp = (((double)getSpTempo())/114.0);
...
//is there a difference in what order wsola nd rate transponder is done?                
    ...
    void speakJetdoCommandWithParameter (int param)
    {
        ...
        else if (getSpCommand() == 22) // master volume
        {
            spPitch = (param);
        }
        else if (getSpCommand() == 23) // bend
        {
            spBend = (param);
        }
        else if (getSpCommand() == 26) // repeat
        {
            spRepeat = (param);
        }
        else if (getSpCommand() == 30) // delay
        {
            // delay in 10ms
            spDelay = param;
            int delay = param;
    ...
    void speakJetAcceptCommand(int command)
    {
        ...
        else if (command == 14) // not implemented
        {
            spNextStress = (true);
        }
        else if (command == 15) // not implemented
        {
            spNextRelax = (true);
        }
    ...
    /**
     * @return the spCommandMode
     */
    public boolean isSpCommandMode() {
        return spCommandMode;
    }
    ...
    public static void speak(final ArrayList<Integer> commands, final boolean isVecVoice)
    {
        ...
        speaker = new Thread() 
        {
            public void run() 
            {
                ...
                for (int i=0; i<vecSpeech.C_LEN; i++) vecSpeech.commands[i] = -1;
