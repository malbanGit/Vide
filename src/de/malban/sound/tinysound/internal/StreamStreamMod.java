 ...
 * @author 
 */
public class StreamStreamMod implements Stream {
	 ...
	 */
	public StreamStreamMod(Mixer mixer, int id, int inputFormat)  
        {
            this.mixer = mixer;
            this.ID = id;
        ...
        // 16 bit signed, 2 channel [big endian] expected (mod data)
	...
	public class StreamStreamReference implements StreamReference 
		...
		private byte[] buf;
                ...
                // 16 bit signed, 2 channel expected (mod data) [mod is bigEndian]
                ...
                public int prepareRead(int countInTinySoundFramesBytes)
                {
                    ...
                    {
                        // mod is BigEndian
                        int inBufferByteHi = inbuffer[i+0]&0xff;//MSB
                        int inBufferByteLo = inbuffer[i+1]&0xff;//LSB
                        
                        //convert it to an int and then to 2 bytes
                        int val = (inBufferByteHi*256) + inBufferByteLo;

                        // left channel
                        this.outBuffer[0+i*(TINYSOUND_FRAME_LENGTH/MOD_FRAME_LENGTH)] = (byte)(val & 0xFF); //LSB
                        this.outBuffer[1+i*(TINYSOUND_FRAME_LENGTH/MOD_FRAME_LENGTH)] = (byte)((val >> 8) & 0xFF); //MSB
                        
                        // mod is BigEndian
                        inBufferByteHi = inbuffer[i+2]&0xff;//MSB
                        inBufferByteLo = inbuffer[i+3]&0xff;//LSB
                        
                        //convert it to an int and then to 2 bytes
                        val = (inBufferByteHi*256) + inBufferByteLo;

                        // right channel
                        this.outBuffer[2+i*(TINYSOUND_FRAME_LENGTH/MOD_FRAME_LENGTH)] = (byte)(val & 0xFF); //LSB
                        this.outBuffer[3+i*(TINYSOUND_FRAME_LENGTH/MOD_FRAME_LENGTH)] = (byte)((val >> 8) & 0xFF); //MSB
                    }              
                    posInOutBuffer=0;
